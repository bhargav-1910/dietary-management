...
class QuotationPopup(ctk.CTkToplevel):
    def __init__(
        ...
        ctk.CTkButton(right, text="Generate", height=40, command=self.generate).grid(
            row=4, column=0, padx=14, pady=(18, 8), sticky="ew"
        )
        ctk.CTkButton(right, text="Preview Bill", height=40, command=self.preview_bill).grid(
            row=5, column=0, padx=14, pady=8, sticky="ew"
        )
        ctk.CTkButton(right, text="Preview Package Sheet", height=40, command=self.preview_package_sheet).grid(
            row=6, column=0, padx=14, pady=8, sticky="ew"
        )
        self.print_button = ctk.CTkButton(right, text="Print", height=40, command=self.print_bill)
        self.print_button.grid(row=7, column=0, padx=14, pady=8, sticky="ew")
        self.download_button = ctk.CTkButton(right, text="Save as PDF", height=40, command=self.download_pdf)
        self.download_button.grid(row=8, column=0, padx=14, pady=8, sticky="ew")
        self.back_button = ctk.CTkButton(right, text="Close Preview", height=40, fg_color="gray", command=self.show_checklist)
        self.back_button.grid(row=9, column=0, padx=14, pady=8, sticky="ew")
        ctk.CTkButton(right, text="Cancel", height=40, fg_color="#b91c1c", hover_color="#991b1b", command=self.destroy).grid(
            row=10, column=0, padx=14, pady=(8, 14), sticky="ew"
        )
    ...
    def _selected_items_and_totals(self) -> tuple[list[dict[str, Any]], dict[str, float]]:
        ...
        for config in self.product_vars.values():
            ...
            qty = int(qty_text) if qty_text.isdigit() and int(qty_text) > 0 else 1
    ...
    def _show_invoice_preview(
        ...
        ctk.CTkLabel(wrap, text=f"Invoice {quotation_info['invoice_number']}", font=ctk.CTkFont(size=22, weight="bold")).grid(
    ...
    def _show_package_preview(self, selected_items: list[dict[str, Any]]) -> None:
        ...
        ctk.CTkLabel(wrap, text="Package Sheet", font=ctk.CTkFont(size=22, weight="bold")).grid(
            row=0, column=0, padx=16, pady=(14, 4), sticky="w"
        )
    ...
    def generate(self) -> None:
        ...
        messagebox.showinfo("Saved", f"Quotation {quotation_info['invoice_number']} saved to patient history.")
    ...
    def print_bill(self) -> None:
        ...
        if preview_type == "invoice":
            ...
            temp_file = os.path.join(tempfile.gettempdir(), f"invoice_{quotation_info['quotation_id']}.pdf")
            file_path = self.pdf_service.generate_invoice_pdf(
                ...
                invoice_number=quotation_info["invoice_number"],
            )
        else:
            selected_items = self.current_preview["selected_items"]
    ...
    def download_pdf(self) -> None:
        if not self.current_preview:
            messagebox.showwarning("No preview", "Preview bill or package sheet first.")
            return
        ...
        if preview_type == "invoice":
            ...
            selected_items = self.current_preview["selected_items"]
            totals = self.current_preview["totals"]
            default_name = f"invoice_{quotation_info['quotation_id']}.pdf"
        else:
            selected_items = self.current_preview["selected_items"]
